...
namespace Jadval.Web.Infrastracture.Helpers
{
    public static class PagedListPagerHelpers
    {
        public static IHtmlContent PagedListPager<T>(this IHtmlHelper htmlHelper, PagedResponse<T> Model, Func<int, string> generatePageUrl)
        {
            ...
            {
                SetLink(ulTag, "Back", generatePageUrl(Model.PageNumber - 1), false);
                if (Model.PageNumber > 5)
                {
                    SetLink(ulTag, "First", generatePageUrl(1), false);
                }
            }
            ...
            if (Model.HasNextPage)
            {
                ...
                {
                    SetLink(ulTag, "End", generatePageUrl(Model.TotalPages), false);
                }
                SetLink(ulTag, "Next", generatePageUrl(Model.PageNumber + 1), false);
            }
